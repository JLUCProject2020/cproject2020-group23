          ...
          waveTimer(nullptr), waveNum(0)
{
    ...
    infoText->setFont(QFont("Georgia", 14, QFont::Bold));
    ...
    menuText->setFont(QFont("Georgia", 12, QFont::Bold));
...
void GameScene::losingGame(std::function<void(void)> functor)
{
    QGraphicsPixmapItem* imgLose = new QGraphicsPixmapItem(gImageCache->load("interface/ZombiesWon.png"));
    QGraphicsPixmapItem* imgShader = new QGraphicsPixmapItem(gImageCache->load("interface/LosingShader.png"));
    QPointF pos = sizeToPoint(sceneRect().size() - imgLose->boundingRect().size()) / 2;
    imgLose->setPos(pos);
    imgLose->setZValue(4);
    imgShader->setZValue(3);
    imgShader->setOpacity(0.25);
    imgLose->setVisible(false);
    imgShader->setVisible(false);
    addItem(imgLose);
    addItem(imgShader);
    ...
    backgroundMusic->play();
    (new Timer(this, 4500, [this, imgLose, imgShader, functor] {
        imgShader->setVisible(true);
        backgroundMusic->stop();
        backgroundMusic->setMedia(QUrl("qrc:/audio/chomp.mp3"));
        backgroundMusic->play();
        (new Timer(this, 400, [this, imgLose, functor] {
            imgLose->setVisible(true);
            backgroundMusic->stop();
            backgroundMusic->setMedia(QUrl("qrc:/audio/scream.mp3"));
            backgroundMusic->play();
            functor();
            }))->start();
        }))->start();
